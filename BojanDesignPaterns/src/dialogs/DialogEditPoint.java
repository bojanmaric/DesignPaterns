...
public class DialogEditPoint extends JDialog {
	...
	public DialogEditPoint(Point p)
	{
		...
		GridBagConstraints gbc_lblEditPoint = new GridBagConstraints();
		gbc_lblEditPoint.insets = new Insets(0, 0, 5, 0);
		gbc_lblEditPoint.gridx = 1;
		gbc_lblEditPoint.gridy = 0;
		jpMainPanel.add(lblEditPoint, gbc_lblEditPoint);
		...
		btnAccept.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				try {
					...
					if (x > 750 || x < 1 || y > 520 || y < 1) {
						throw new NumberFormatException();
					}
					dispose();
				} catch (NumberFormatException ex) {
					JOptionPane.showMessageDialog(DialogEditPoint.this, "Only numbers 1-750 for X and 1-520 for Y allowed",
